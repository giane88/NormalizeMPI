 ...
 */
Image::Image(unsigned char newMin, unsigned char newMax,
		mpi::communicator world) :
		newMin(newMin), newMax(newMax), world(world) {
	minRed = 255;
	minGreen = 255;
	minBlue = 255;
	maxRed = 0;
...
void Image::loadImage(std::string path) {
	...
	if (world.rank() == 0) {
				...
				if ((line.compare("P3") == 0) && (nline == 0)) {
					...
					if (line.compare(0, 1, "#") == 0) {
						...
						if (nline == 1) {
							...
							std::cout << "Processo " << world.rank()
									<< ": Invio dimensioni..." << std::endl;
		...
		inputfile.close();
		std::cout << "Processo " << world.rank() << ": Invio matrice..."
				<< std::endl;
		std::cout.flush();
		broadcast(world, immatrix, 0);
	} else {
		std::cout << "Processo: " << world.rank() << ": In attesa di ricezione"
				<< std::endl;
		broadcast(world, altezza, 0);
		std::cout << "Ricevuta altezza: " << altezza << std::endl;
		broadcast(world, larghezza, 0);
		std::cout << "Ricevuta altezza: " << larghezza << std::endl;
		broadcast(world, immatrix, 0);
		std::cout << "Processo " << world.rank() << ": Ricezione avvenuta..."
				<< std::endl;
		std::cout.flush();
	}
}
...
void Image::fillMatrix() {
	...
	for (i = 0; i < larghezza; i++) {
		...
		for (j = 0; j < altezza; j++) {
			...
			temp.push_back(
					Pixel((unsigned char) atoi(red.c_str()),
							(unsigned char) atoi(green.c_str()),
							(unsigned char) atoi(blue.c_str())));
		}
...
void Image::testPrint() {
	...
	for (i = 0; i < larghezza; i++) {
		for (j = 0; j < altezza; j++) {
			std::cout.width(3);
			std::cout << int(immatrix[i][j].getRed()) << " "
					<< int(immatrix[i][j].getGreen()) << " "
					<< int(immatrix[i][j].getBlue()) << "\t";
			std::cout.flush();
		}
		std::cout << std::endl;
	}
...
void Image::findMin() {
	...
	std::cout << "Processo " << world.rank() << ": Eseguo il ciclo da "
			<< ((world.rank() * larghezza) / world.size()) << " a "
			<< (((world.rank() + 1) * larghezza) / world.size()) << std::endl;
	std::cout.flush();
#pragma omp parallel for
	for (i = ((world.rank() * larghezza) / world.size());
			i < (((world.rank() + 1) * larghezza) / world.size()); i++) {
#pragma omp parallel for
		for (j = 0; j < altezza; j++) {
#pragma omp parallel
			{
				if (immatrix[i][j].getRed() < l_minRed)
					l_minRed = immatrix[i][j].getRed();
				if (immatrix[i][j].getGreen() < l_minGreen)
					l_minGreen = immatrix[i][j].getGreen();
				if (immatrix[i][j].getBlue() < l_minBlue)
					l_minBlue = immatrix[i][j].getBlue();
			}
		}
	}
	...
	if (world.rank() == 0) {
		...
		std::cout << "Processo " << world.rank()
				<< ": I valori minimi per RGB sono: " << int(minRed) << " "
				<< int(minGreen) << " " << int(minBlue) << std::endl;
	} else {
...
#pragma omp parallel
			{
				if (immatrix[i][j].getRed() > l_maxRed)
					l_maxRed = immatrix[i][j].getRed();
				if (immatrix[i][j].getGreen() > l_maxGreen)
					l_maxGreen = immatrix[i][j].getGreen();
				if (immatrix[i][j].getBlue() > l_maxBlue)
					l_maxBlue = immatrix[i][j].getBlue();
			}
		}
	}
	...
	std::cout << "Processo " << world.rank()
			<< ": I valori massimi per RGB sono: " << int(maxRed) << " "
			<< int(maxGreen) << " " << int(maxBlue) << std::endl;
}
...
#pragma omp parallel
			{
				if ((maxRed - minRed) != 0) {
					immatrix[i][j].setRed(
							(((immatrix[i][j].getRed() - minRed)
									* (newMax - newMin)) / (maxRed - minRed))
									+ newMin);
				}
				if ((maxGreen - minGreen) != 0) {
					immatrix[i][j].setGreen(
							(((immatrix[i][j].getGreen() - minGreen)
									* (newMax - newMin)) / (maxGreen - minGreen))
									+ newMin);
				}
				if ((maxBlue - minBlue) != 0) {
					immatrix[i][j].setBlue(
							(((immatrix[i][j].getBlue() - minBlue)
									* (newMax - newMin)) / (maxBlue - minBlue))
									+ newMin);
				}
			}
		}
...
void Image::receiveMatrix() {
	...
	if (world.rank() != 0) {
		...
		for (j = 1; j < world.size(); j++) {
			for (i = ((j * larghezza) / world.size());
					i < (((j + 1) * larghezza) / world.size()); i++) {
				std::vector<Pixel> temp;
				world.recv(j, i, temp);
				immatrix[i].clear();
				immatrix[i] = temp;
